...
Thank you!`,a="6281188030203",r=encodeURIComponent(i),c=`https://wa.me/${a}?text=${r}`;window.open(c,"_blank")}function openEventModal(e,t,n,s){const o=document.createElement("div");o.className="event-modal";function i(e){let n=e.replace(/\\n/g,`
`).replace(/\r\n/g,`
`).replace(/\r/g,`
`),s=n.split(`

`).map(e=>e.trim()).filter(e=>e!==""),t="";for(let e of s)if(e.includes("•")){let s=e.split(`
`).map(e=>e.trim()).filter(e=>e!==""),n="";for(let e of s)if(e.startsWith("•")){let s=e.substring(1).trim(),t=s.match(/^\*\*(.*?)\*\*\s*-\s*(.*)$/);t?n+=`<div class="bullet-item">
                <div class="bullet-title">${t[1]}</div>
                <div class="bullet-description">${t[2]}</div>
              </div>`:n+=`<div class="bullet-item">
                <div class="bullet-description">${s}</div>
              </div>`}else{let t=e.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>");e.match(/^\*\*.*\*\*$/)?n+=`<div class="section-header">${t}</div>`:n+=`<p>${t}</p>`}t+=`<div class="bullet-section">${n}</div>`}else{let n=e.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/\n/g,"<br>").replace(/\t/g,"&nbsp;&nbsp;&nbsp;&nbsp;").replace(/  /g,"&nbsp;&nbsp;");t+=`<p>${n}</p>`}return t}o.innerHTML=`
